...
function ConfirmationContent() {
  ...
  const paymentMethod = searchParams.get("payment_method") as "bank_transfer" | "cash_on_delivery" | "qpay" | null;

  const [order, setOrder] = useState<Order | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [copiedField, setCopiedField] = useState<string | null>(null);
  // Track if we have full order data or just showing success without details
  const [hasFullOrderData, setHasFullOrderData] = useState(false);

  // Clear cart from store after successful checkout
  const { clearCart } = useCartStore();

  // Clear cart immediately on mount (order completed)
  useEffect(() => {
    clearCart();
  }, [clearCart]);
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      // If no order ID provided, just show success without order details
      if (!orderId) {
        setIsLoading(false);
        setHasFullOrderData(false);
        return;
      }
      ...
      try {
        ...
        if (fetchedOrder) {
          ...
          setHasFullOrderData(true);
        } else {
          // Order not found but we have an ID - show success without details
          setHasFullOrderData(false);
        }
      } catch (err: unknown) {
        console.error("Error fetching order:", err);
        // For any error (auth, not found, etc.), just show success without details
        // The order was created, we just can't retrieve it
        setHasFullOrderData(false);
        setError(null);
      } finally {
        setIsLoading(false);
  ...
  return (
    <main className="flex-1 py-8 sm:py-16">
      <div className="max-w-2xl mx-auto px-4 sm:px-6">
        {/* Success Header - Professional Design */}
        <div className="text-center mb-10">
          {/* Animated Success Icon */}
          <div className="relative inline-flex items-center justify-center mb-8">
            {/* Outer ring with gradient */}
            <div className="absolute inset-0 w-24 h-24 rounded-full bg-linear-to-br from-[#34c759]/20 to-[#30d158]/10" />
            {/* Inner circle */}
            <div className="relative w-20 h-20 rounded-full bg-linear-to-br from-[#34c759] to-[#30d158] flex items-center justify-center shadow-lg shadow-[#34c759]/25">
              <Check className="w-10 h-10 text-white stroke-3" />
            </div>
          </div>
          
          <h1 className="text-[32px] sm:text-[40px] font-bold text-[#1d1d1f] mb-3 tracking-tight">
            Захиалга амжилттай!
          </h1>
          <p className="text-[17px] sm:text-[19px] text-[#86868b] max-w-md mx-auto">
            Баярлалаа! Таны захиалга амжилттай хүлээн авлаа.
          </p>
        </div>

        {/* Order Number - Only show if we have full order data */}
        {hasFullOrderData && order && (
          <div className="bg-white rounded-2xl p-6 mb-4 text-center border border-gray-100 shadow-sm">
            <p className="text-[13px] uppercase tracking-wider text-[#86868b] mb-2 font-medium">Захиалгын дугаар</p>
            <p className="text-[32px] font-bold text-[#1d1d1f] tracking-tight">
              #{order.display_id}
            </p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-[14px] text-[#86868b] mt-1">
                  Бараа хүлээн авахдаа хүргэлтийн ажилтанд бэлнээр эсвэл QPay-ээр төлнө үү.
                  {hasFullOrderData && order && (
                    <span className="block mt-1 font-semibold text-[#1d1d1f]">
                      Төлөх дүн: {formatPrice(order.total, order.currency_code)}
                    </span>
                  )}
                </p>
              </div>
        ...
        {isPickup && (
          <div className="bg-[#0071e3]/5 border border-[#0071e3]/20 rounded-2xl p-6 mb-4">
            ...
            {paymentMethod === "cash_on_delivery" && hasFullOrderData && order && (
        ...
        {hasFullOrderData && order && order.items && order.items.length > 0 && (
          <div className="bg-white rounded-2xl p-6 mb-4 border border-gray-100 shadow-sm">
            <h2 className="text-[17px] font-semibold text-[#1d1d1f] mb-4">
              Захиалсан бараа
            </h2>
        ...
        {/* What's Next Section - Show when we don't have full order data */}
        {!hasFullOrderData && (
          <div className="bg-white rounded-2xl p-6 mb-4 border border-gray-100 shadow-sm">
            <div className="flex items-start gap-3 mb-4">
              <div className="w-10 h-10 rounded-full bg-[#0071e3]/10 flex items-center justify-center shrink-0">
            ...
            <ul className="space-y-3 text-[14px] text-[#1d1d1f]">
              <li className="flex items-start gap-2">
                <div className="w-5 h-5 rounded-full bg-[#34c759]/10 flex items-center justify-center shrink-0 mt-0.5">
                  <Check className="w-3 h-3 text-[#34c759]" />
                </div>
                <span>Захиалга баталгаажуулах имэйл илгээгдэнэ</span>
              </li>
              <li className="flex items-start gap-2">
                <div className="w-5 h-5 rounded-full bg-[#34c759]/10 flex items-center justify-center shrink-0 mt-0.5">
                  <Check className="w-3 h-3 text-[#34c759]" />
                </div>
                <span>Бид таны захиалгыг боловсруулж эхэлнэ</span>
              </li>
              <li className="flex items-start gap-2">
                <div className="w-5 h-5 rounded-full bg-[#34c759]/10 flex items-center justify-center shrink-0 mt-0.5">
                  <Check className="w-3 h-3 text-[#34c759]" />
                </div>
                <span>Захиалгын явцын талаар мэдэгдэл илгээнэ</span>
              </li>
            </ul>
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-8">
          <Link
            href="/products"
            className="flex-1 bg-[#0071e3] text-white rounded-xl py-4 px-6 font-semibold text-[17px] hover:bg-[#0077ed] transition-all text-center flex items-center justify-center gap-2 shadow-lg shadow-[#0071e3]/20"
          >
          ...
          <Link
            href="/account/orders"
            className="flex-1 bg-white border border-gray-200 text-[#1d1d1f] rounded-xl py-4 px-6 font-semibold text-[17px] hover:bg-gray-50 transition-all text-center shadow-sm"
          >
            Захиалгууд харах
          </Link>
        </div>
