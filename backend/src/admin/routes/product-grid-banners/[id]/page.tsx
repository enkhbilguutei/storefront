...
const EditProductGridBannerPage = () => {
  const { id } = useParams()
      ...
      try {
        ...
        if (data.banner) {
          setFormData({
            ...
            mobile_image_url: data.banner.mobile_image_url || "",
            link: data.banner.link || "",
            alt_text: data.banner.alt_text || "",
            placement: data.banner.placement || "product_grid",
            section: data.banner.section || "apple",
            sort_order: data.banner.sort_order || 1,
            is_active: data.banner.is_active ?? true,
            dark_text: data.banner.dark_text ?? false,
          })
        }
      } catch (error) {
  ...
  return (
    <Container className="divide-y p-0">
      <div className="flex items-center justify-between px-6 py-4">
        <div>
          <Heading level="h1">Grid Баннер засах</Heading>
          <p className="text-ui-fg-subtle text-sm mt-1">
            Бүтээгдэхүүний хэсэгт харагдах босоо баннер засах
          </p>
        </div>
      </div>
...
export const config = defineRouteConfig({
  label: "Grid Баннер засах",
})

export default EditProductGridBannerPage
